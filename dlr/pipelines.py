...
class DlrPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('dlr.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dlr`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dlr WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dlr (date, title, link, content)"
